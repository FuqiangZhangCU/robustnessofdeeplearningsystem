...
class CIFAR:
    def __init__(self):
        ...
        if not os.path.exists("inputs/data/cifar-10-batches-bin"):
            urllib.request.urlretrieve("https://www.cs.toronto.edu/~kriz/cifar-10-binary.tar.gz",
                                       "inputs/data/cifar-data.tar.gz")
            os.popen("tar xzf inputs/data/cifar-data.tar.gz -C inputs/data/").read()
            

        for i in range(5):
            r,s = load_batch("inputs/data/cifar-10-batches-bin/data_batch_"+str(i+1)+".bin")
            train_data.extend(r)
        ...
        self.test_data, self.test_labels = load_batch("inputs/data/cifar-10-batches-bin/test_batch.bin")
